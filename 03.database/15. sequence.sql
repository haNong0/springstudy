...
/*
시퀀스란? 순차적으로 증감하는 일련번호를 생성하는 오라클 데이터베이스의 객체이다.
보통 PK값에 중복을 방지하기 위해서 종종 사용한다. 에를 들어 게시판에 글이 하나가 추가될 때
마다 글번호(PK)가 생겨야 한다면 시퀀스를 사용하면 보다 쉽게, 편리하게 PK를 관리할 수가 있다.

유일한 값을 생성해 주는 오라클 객체중 하나다.
시퀀스를 생성하면 기본키와 같이 순차적으로 증가하는 컬럼을 자동으로 생성이 가능하다.
...
[increment by n]				 	-- 생략하면 기본값1
[maxvalue n | nomaxalue]  --생략하면 999999
[minvalue | nominvalue]		--생략하면 기본값 nominvalue
[cycle |no nocylce]				-- 생략하면 기본값 nocyle
[cache | nocache]					-- 생략하면 기본값 nocache

... start with : 시퀀스의 시작  정의를 n을 1000이라 지정하면 1000부터 시작 정의하지 않으면 1부터 시작
... increment by : 자동증가 값을 정의, n의 10이면 10씩 증가
...
select jno_seq.currval from dual; --sequence JNO_SEQ.CURRVAL is not yet defined in this session
...
insert into s_order values(jno_seq.nextval, '거미', 'mp3', 1);
select jno_seq.currval from dual;
select * from s_order;
