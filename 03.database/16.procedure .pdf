...
/*
in, out 매개변수가 있는 프로시저 생성하기
create or replace procedure  pro_sal_raise(p_deptno in number, p_percent) is
*/
--in out 매개변수
--사원 번호를 전달 받아서 사원명과 급여, 직책을 리턴 procedure
...
select ename, sal, job, 
	...
	v_job		varchar2(20);
	begin
	-- 프로시저exec,excute,call을 사용할 수 없고
	-- 포로시저명으로 호출해야한다.
	emp_sal_job(7369, v_ename, v_sal, v_job);
	dbms_output.put_line('사원이름='||v_name||chr(9)||',급여'v_sal||chr(9)||',직급'||v_job);
	end;
	
	select emp_sal_job(sal);
